...
> **Поправка 17.08.2026.** Раньше здесь было написано «в Strapi 29 компонентов DZ,
> в свитче 31 ветка». Пересчитал по коду: уникальных `page.*` в свитче ровно **29**,
> расхождения нет, контракт целый. Ошибка была моя — цифру взял из отчёта субагента,
> не проверив. Класс К-14 наоборот: принял чужой замер за свой. Это готовый механизм «тихого» дефекта: контент есть, на странице
