/**
 * Скрипт для создания страницы main_page в Strapi на основе main_page.txt
 */
...
const CONTENT_FILE = path.join(__dirname, '../../main_page.txt');
...
async function findPageBySlug(slug) {
  try {
    ...
    console.error('[Create] Ошибка при поиске страницы:', error.message);
    return null;
  }
}
  ...
  try {
    ...
    if (existingPage) {
      ...
      try {
        ...
        if (pubError.response) {
          console.warn('[Create] Response:', JSON.stringify(pubError.response.data, null, 2));
        }
      }
...
async function main() {
  ...
  console.log('\n💡 Теперь главная страница должна загружаться из CMS без ошибки 404!');
}
