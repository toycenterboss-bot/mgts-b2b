...
/**
 * Создать правильно отформатированный HTML формы заказа
 */
function createFormattedOrderFormHTML() {
...
/**
 * Исправить форматирование формы на странице
 */
function fixFormFormatting(htmlContent, slug) {
    // Парсим HTML
    ...
    if (serviceOrder.length === 0) {
        log(`   ⚠️  Форма не найдена, добавляю...`);
        // Если формы нет, добавляем её
        const newFormHTML = createFormattedOrderFormHTML();
        ...
        serviceOrder.each((i, section) => {
            ...
            // Проверяем, есть ли container внутри
            const container = $section.find('.container');
            if (container.length === 0) {
                log(`   🔧 Container отсутствует, исправляю структуру...`);
                // Если container нет, оборачиваем содержимое в container
                const content = $section.html();
                $section.html(`<div class="container">${content}</div>`);
                modified = true;
            }
            
            // Проверяем, правильно ли размещена форма
            const $parent = $section.parent();
            const parentTag = $parent.get(0)?.tagName || '';
            
            // Если форма внутри неправильного контейнера, перемещаем её
            if (parentTag === 'div' && !$parent.hasClass('service-order')) {
                ...
                modified = true;
            } else if (parentTag !== 'body' && parentTag !== 'html') {
                // Если родитель не body и не html, перемещаем
                log(`   🔧 Форма внутри ${parentTag}, перемещаю на верхний уровень...`);
                const sectionHTML = $section[0].outerHTML || '';
                $section.remove();
                $.root().append(sectionHTML);
        ...
        // Если структура правильная, но нужно обновить форматирование
        if (!modified) {
            // Заменяем форму на правильно отформатированную версию
            log(`   🔧 Обновляю форматирование формы...`);
            const newFormHTML = createFormattedOrderFormHTML();
            serviceOrder.replaceWith(newFormHTML);
            modified = true;
        }
    }
    ...
    if (modified) {
        ...
        // Удаляем автоматически добавленные теги html/head/body
        if (updatedHTML.includes('<html>')) {
            const bodyMatch = updatedHTML.match(/<body[^>]*>([\s\S]*)<\/body>/i);
            if (bodyMatch) {
                updatedHTML = bodyMatch[1];
            } else {
                updatedHTML = updatedHTML.replace(/^[\s\S]*?<body[^>]*>/, '');
                updatedHTML = updatedHTML.replace(/<\/body>[\s\S]*$/, '');
            }
        }
...
/**
 * Исправить форматирование форм на всех проблемных страницах
 */
async function fixAllForms() {
    const problemSlugs = ['business', 'developers', 'government', 'index', 'operators', 'partners'];
    
    log('\n' + '='.repeat(70));
    log('🎨 ИСПРАВЛЕНИЕ ФОРМАТИРОВАНИЯ ФОРМ');
    log('='.repeat(70));
    ...
    for (let i = 0; i < problemSlugs.length; i++) {
        ...
        try {
            ...
            // Исправляем форматирование формы
            const { html: updatedContent, modified } = fixFormFormatting(content, slug);
            ...
            if (modified) {
                ...
                if (updateResponse.ok) {
                    log(`   ✅ Форматирование формы исправлено`);
...
module.exports = { fixFormFormatting };
