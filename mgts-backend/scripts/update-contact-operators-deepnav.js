/**
 * Create/refresh deep-nav tree for contact_for_operators and set page deepNavKey.
 *
 * Usage:
 *   STRAPI_API_TOKEN=... node update-contact-operators-deepnav.js
 *
 * Env:
...
const DEEPNAV_KEY = 'operators_contacts';
const PAGE_SLUG = 'contact_for_operators';
...
async function main() {
  ...
  const sidebar = (spec.sections || []).find((s) => s?.type === 'sidebar');
  const links = sidebar?.links?.internalLinks || [];
  if (!links.length) {
    throw new Error('Sidebar links not found in spec.');
  }
  ...
  const existingKeys = ['contacts', 'documents', 'about_company', 'disclosure'];
  const existingTrees = [];
  ...
  await api.put(`/pages/${pageId}`, {
    data: { template: 'TPL_DeepNav', deepNavKey: DEEPNAV_KEY },
  });

  console.log(`✅ DeepNav updated: ${DEEPNAV_KEY} and page template set to TPL_DeepNav`);
}
