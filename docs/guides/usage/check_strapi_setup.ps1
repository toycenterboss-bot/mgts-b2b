...
if (Test-Path $nodeModulesPath) {
    ...
    $strapiPath = Join-Path $nodeModulesPath "@strapi"
    if (Test-Path $strapiPath) {
        Write-Host "   [OK] Strapi installed in node_modules" -ForegroundColor Green
        
        # Check for main strapi package
        $strapiMainPath = Join-Path $strapiPath "strapi"
        if (Test-Path $strapiMainPath) {
            Write-Host "   [OK] @strapi/strapi package found" -ForegroundColor Green
        } else {
            Write-Host "   [INFO] Checking for Strapi packages..." -ForegroundColor Gray
            $strapiPackages = Get-ChildItem -Path $strapiPath -Directory -ErrorAction SilentlyContinue
            if ($strapiPackages) {
                Write-Host "   [OK] Found $($strapiPackages.Count) Strapi packages" -ForegroundColor Green
            }
        }
    } else {
...
if (Test-Path $configPath) {
    ...
    # Check for database.js or database.ts (TypeScript support)
    $dbConfigPathJs = Join-Path $configPath "database.js"
    $dbConfigPathTs = Join-Path $configPath "database.ts"
    if (Test-Path $dbConfigPathJs) {
        Write-Host "   [OK] database.js exists" -ForegroundColor Green
    } elseif (Test-Path $dbConfigPathTs) {
        Write-Host "   [OK] database.ts exists (TypeScript)" -ForegroundColor Green
    } else {
        Write-Host "   [WARNING] database.js/ts not found" -ForegroundColor Yellow
        $warnings += "database config not found"
    }
    
    # Check for server.js or server.ts (TypeScript support)
    $serverConfigPathJs = Join-Path $configPath "server.js"
    $serverConfigPathTs = Join-Path $configPath "server.ts"
    if (Test-Path $serverConfigPathJs) {
        Write-Host "   [OK] server.js exists" -ForegroundColor Green
    } elseif (Test-Path $serverConfigPathTs) {
        Write-Host "   [OK] server.ts exists (TypeScript)" -ForegroundColor Green
    } else {
        Write-Host "   [WARNING] server.js/ts not found" -ForegroundColor Yellow
        $warnings += "server config not found"
    }
} else {
