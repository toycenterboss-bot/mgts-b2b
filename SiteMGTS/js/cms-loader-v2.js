...
(function() {
    ...
    class CMSLoader {
        ...
        async load(slug = null) {
            ...
            // Установить атрибут data-cms-loaded для совместимости
            const mainContent = document.querySelector(CONFIG.selectors.mainContent);
            if (mainContent) {
                mainContent.setAttribute('data-cms-loaded', 'true');
            }
        ...
        /**
         * Рендеринг главного меню (для совместимости с navigation-renderer.js)
         * @param {Object} menuData - Данные меню из Strapi API
         */
        renderMainMenu(menuData) {
            // Если menuData - это HTML строка, просто вставляем её в #mainNav
            ...
            // Если menuData - это объект с данными, используем navigation-renderer
            // NavigationRenderer.renderMainMenuFromMenuData ожидает контейнер (body или header)
            const container = document.body || document.querySelector('header')?.parentElement || document.body;
            
            // Проверяем, есть ли NavigationRenderer в глобальной области
            // Он может быть доступен через замыкание или через window
            if (typeof renderMainMenuFromMenuData === 'function') {
                // Функция доступна в глобальной области (из navigation-renderer.js)
                renderMainMenuFromMenuData(menuData, container);
            } else if (window.NavigationRenderer && typeof window.NavigationRenderer.renderMainMenuFromMenuData === 'function') {
                window.NavigationRenderer.renderMainMenuFromMenuData(menuData, container);
            } else {
                // Пробуем найти функцию через eval (не рекомендуется, но для совместимости)
                console.warn('[CMS Loader] NavigationRenderer.renderMainMenuFromMenuData not available, menu will be rendered by navigation-renderer.js directly');
            }
        }

        /**
         * Перезагрузить контент (для совместимости)
         */
        reload() {
            this.isLoaded = false;
            this.isLoading = false;
         ...
         */
        clearCache() {
            if (window.StrapiAPI && typeof window.StrapiAPI.clearCache === 'function') {
