...
export default function ServiceOrderForm({ section }: ServiceOrderFormProps) {
  if (section?.isVisible === false) return null;
  const method = (section.formMethod || "POST").toUpperCase();
  const badgeText = section?.badgeText;
  const title = section?.title;
  const subtitle = section?.subtitle;
  const phoneLabel = section?.supportPhoneLabel || section?.phoneLabel;
  const phoneValue = section?.supportPhoneValue || section?.phoneValue;
  const emailLabel = section?.supportEmailLabel || section?.emailLabel;
  const emailValue = section?.supportEmailValue || section?.emailValue;
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="flex flex-col gap-6">
              ...
              {(phoneLabel && phoneValue) || (emailLabel && emailValue) ? (
